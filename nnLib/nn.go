...
func (g *Graph) BackProp(root int, rebuild bool) {
	...
	for i := len(g.order) - 1; i >= 0; i-- {
		...
		case Add:
			g.Values[curRoot.idx2].Gradient += curRoot.Gradient
			g.Values[curRoot.idx1].Gradient += curRoot.Gradient

		case Exp:
			g.Values[curRoot.idx2].Gradient += curRoot.Data * curRoot.Gradient

		case Mul:
			x := g.GetValue(curRoot.idx2)
		...
		case ReLU:
			...
			g.Values[curRoot.idx2].Gradient += base * curRoot.Gradient

		case TanH:
			tanH := curRoot.Data
			g.Values[curRoot.idx2].Gradient += (1 - tanH*tanH) * curRoot.Gradient
		}
	}
}
...
func (g *Graph) ForwardProp(root int, rebuild bool) {
	...
	for i := 0; i < len(g.order); i++ {
		...
		switch curRoot.OpType {
		case Add:
			curRoot.Data = g.GetData(curRoot.idx2) + g.GetData(curRoot.idx1)

		case Exp:
			curRoot.Data = math.Exp(g.GetData(curRoot.idx2))

		case Mul:
			curRoot.Data = g.GetData(curRoot.idx2) * g.GetData(curRoot.idx1)

		case Pow:
			curRoot.Data = math.Pow(g.GetData(curRoot.idx2), g.powers[g.order[i]])

		case ReLU:
			data := g.GetData(curRoot.idx2)
			if data < 0 {
				data = 0
			}
